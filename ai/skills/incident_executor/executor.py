...
class IncidentState:
    ...
    def __init__(self):
        ...
        self.current_acl_name: str = ""  # 当前ACL策略名称
...
""")
    ...
    if tool_name == "acl_policy_ban":
        # 写死的 Cisco IOS 命令执行输出
        cmd_output = """sw>en
Password:*****
sw#config
Enter configuration commands, one per line.  End with CNTL/Z.
sw(config)#interface range gigabitEthernet 0/1-12
sw(config-if-range)#no ip access-group 30 in
sw(config-if-range)#ip access-group udp in
...
4. 用户确认后调用 acl_policy_ban 执行封禁（使用Cisco风格ACL命令，如 'ip access-list extended udp' 进入配置模式，'20 deny udp any any eq 4750' 添加规则）

## 极域漏洞处置（重要）
当用户说"修复"时，**立即调用 acl_policy_ban 工具**执行封禁
**不需要询问用户，直接执行封禁！**

## 重要说明
- 收到任务后**先分析事件内容，输出执行计划表格**不显示工具，并询问用户"是否开始执行？"
- 用户确认后，才开始调用工具
- 每步工具执行完毕看结果再决定下一步工具
- 所有工具调用完毕后才输出总结文字
- 报告生成后必须询问用户是否执行封禁策略
- **报告中禁止出现工具名称（如 acl_policy_ban、get_traffic_logs 等），只描述操作动作和结果**
...
def create_incident_stream(
    ...
    while state.step_count < state.max_steps and not state.is_complete:
        ...
        tool_descriptions = {
            ...
            "acl_policy_ban": "acl封禁配置，使用Cisco风格ACL命令执行封禁，如 'ip access-list extended udp' 进入配置模式，或 '20 deny udp any any eq 4750' 添加规则",
        }
