...
3. **AICenter.vue（AI 中枢）**
   ...
   - **AI 对话（Chat）**:
     ...
     - **追问机制**: 支持多轮对话，深入分析问题。
     - **TTS 语音按钮**: 位于聊天输入框底部、发送（回车）按钮旁边。展示为一个麦克风 icon 按钮；激活时显示声音波浪动画表示正在录音/播报。若浏览器需要麦克风权限，必须先弹窗提示用户授权后再开始录音。
   - **AI 报告（Report）**:
...
- 组件结构
  - 会话面板：消息流、证据引用、上下文标签。
  - 报告面板：模板选择、时间范围、导出格式。
  - TTS 按钮：位于聊天输入框底部发送按钮旁，麦克风 icon + 声音波浪动画；需要麦克风权限时先弹窗提示授权。
- 数据接口
...
## 成品化补充清单（按优先级）
### P0（必须先补齐）
- [x] 可运行工程骨架：`backend/`、`frontend/`、`requirements.txt`、初始化脚本、样例配置可直接启动。
- [x] 数据库可执行规范：建表 SQL、索引、唯一约束、状态枚举、初始化数据、迁移策略（建议 Alembic）。
- [x] API 契约冻结：OpenAPI 文档、错误码映射、幂等规则、分页与排序标准统一。
- [x] 关键任务状态机：`threat_event`、`execution_task`、`scan_task`、`firewall_sync_task` 的状态流转、超时与人工接管规则明确。
- [x] 安全最小闭环：凭据加密、密钥管理、RBAC 接口权限矩阵、审计日志追加写规则可执行。

### P1（上线前补齐）
- [x] 可观测性：结构化日志、`trace_id` 全链路透传、指标与告警阈值落地。
- [x] 测试与质量门禁：单测/集成/E2E 覆盖目标、CI 流水线（lint + test + build）与阻断规则。
- [x] 部署分层：`dev/test/prod` 环境配置分离、反向代理与密钥注入方案、备份恢复流程。

### P2（稳定运营优化）
- [x] 验收 KPI 量化：误报率、处置 SLA、封禁成功率、报告生成时延按周期复盘。
- [x] 外部联动治理：MCP 插件准入清单、风险分级、熔断与降级策略、第三方故障隔离。
...
- [x] OpenAPI 与实现一致，变更日志齐全。
- [x] 数据迁移脚本在测试环境演练通过。
- [x] CI 全绿（lint/test/build/e2e）。
- [x] SLO 仪表盘与告警规则已启用。
- [x] 回滚脚本、应急联系人、值班手册已就绪。
- [x] 关键高风险动作已验证二次确认与审计留痕。
...
- [x] `X-01` 测试体系
  - [x] 单元测试：DSL 解析、校验器、状态机。
  - [x] 集成测试：API + DB + adapter。
  - [x] E2E：可视化编辑到发布到运行到回滚。
  - [x] 回归基线：旧接口行为不可回归。

- [x] `X-02` 性能与稳定性
  - [x] 压测：并发发布、并发运行、长链路重试。
  - [x] 指标：P95 延迟、任务积压、失败重试率。
  - [x] 保护：熔断、限流、隔离队列。

- [x] `X-03` 运维与发布
  - [x] Feature Flag：分阶段开关。
  - [x] 灰度/回滚预案文档。
  - [x] 运行手册：告警、排障、应急处置。

##### 周计划（执行节奏建议）

- [x] Week 1：完成 M1（Schema/表/API/只读图）+ 单测。
- [x] Week 2：完成 M2（编辑/校验/发布回滚/RBAC）+ 集成测试。
- [x] Week 3：完成 M3（编排器 + defense 灰度）+ 回退演练。
- [x] Week 4：完成 M4（scan 接入 + 监控 + 回放）+ 全量回归。

##### 每个任务统一完成标准（模板）

- [x] 代码已合入并通过 `lsp_diagnostics`。
- [x] 对应测试新增并通过（单元/集成/E2E 至少其一，关键链路需全覆盖）。
- [x] README/API 文档已更新。
- [x] 审计日志可追踪（含 `trace_id`）。
- [x] 提供回滚方案并验证可执行。
...
- [x] 5.1 实现 `ai_chat`：会话创建、上下文拼装、消息入库。
- [x] 5.2 实现 `report`：摘要版/详细版报告生成与导出。
- [x] 5.3 实现 `tts`：任务提交、状态查询、音频索引记录。
- [x] 5.4 在 `AICenter` 打通对话、报告、TTS 三个入口。
- [x] 5.5 验证：可对同一事件/任务发起问答并导出报告。

### Step 6：外部联动（插件/推送/防火墙）✅
- [x] 6.1 实现 `plugin_registry` 管理与启停接口。
- [x] 6.2 实现 `push_channel`（至少 1 种通道）。
- [x] 6.3 实现防火墙同步接口与签名校验。
- [x] 6.4 实现防火墙幂等键（`Idempotency-Key` -> `request_hash`）。
- [x] 6.5 实现回执处理、失败重试、`MANUAL_REQUIRED` 转人工。
- [x] 6.6 验证：封禁后可触发防火墙同步并正确记录回执。

### Step 7：安全加固（上线前必须完成）✅
- [x] 7.1 实现凭据加密存储（AES-GCM）与 `key_version`。
- [x] 7.2 实现密钥轮换流程（旧密钥可解，新密钥重加密）。
- [x] 7.3 高风险动作添加二次确认与操作原因必填。
- [x] 7.4 增加接口限流、防暴力登录、白名单策略。
- [x] 7.5 完善审计日志不可篡改策略（追加写 + 校验）。
- [x] 7.6 验证：随机抽检高风险动作，审计字段齐全且可追溯。
...
- [x] 9.4 前端关键交互测试：审批、状态刷新、错误提示。
- [x] 9.5 新增后台壳 E2E：Sidebar 路由切换、Topbar 视图切换、右上角退出登录。
- [x] 9.6 新增权限 E2E：viewer/operator/admin 菜单可见性与接口访问控制。
- [x] 9.7 新增系统功能 E2E：通知已读、全局搜索跳转、探测任务触发审计写入。
- [x] 9.8 CI 接入：lint + test + build + e2e。
- [x] 9.9 阻断规则：任一关键流程失败禁止合并。
- [x] 9.10 验证：主分支流水线连续 3 次通过。

### Step 10：预发布与上线
- [x] 10.1 完成数据迁移演练与回滚演练。
- [x] 10.2 完成压测（API、队列、扫描并发）并输出报告。
- [x] 10.3 完成故障演练（MCP/AI/防火墙不可用）。
- [x] 10.4 完成值班手册、应急预案、联系人清单。
- [x] 10.5 小流量灰度发布并观察核心指标。
- [x] 10.6 正式放量上线并冻结版本。
- [x] 10.7 验证：上线后一周无 P0/P1 故障，SLO 达标。

## 每步完成定义（DoD 模板）
- [x] 代码已合并且 CI 通过。
- [x] 关键日志与审计字段完整。
- [x] 文档已同步更新（接口、状态机、配置项）。
- [x] 失败分支已验证（至少 1 个异常场景）。
- [x] 有可复现的验证记录（命令输出/截图/测试报告）。

## 后台功能专项排期（两周冲刺模板）
### Sprint A（第 1 周）：后台壳与系统能力 ✅
- Day 1
  - [x] 建立 App Shell、路由骨架、权限路由守卫。
  - [x] 接入 `useAuthStore/useAppStore`。
- Day 2
  - [x] 完成 Sidebar 菜单、权限可见性、激活高亮。
  - [x] 完成 Topbar 基础布局与系统状态显示。
- Day 3
  - [x] 完成防御/探测视图切换 UI（仅展示层）。
  - [x] 打通手动探测任务触发入口与回执展示。
- Day 4
  - [x] 完成右上角菜单：通知、个人中心、安全设置、退出登录。
  - [x] 打通 `POST /auth/logout` 并验证会话清理。
- Day 5
  - [x] 完成 `overview` 页面卡片、趋势、待办模块。
  - [x] 执行 Sprint A 回归（菜单切换/视图切换/退出登录/审计）。

### Sprint B（第 2 周）：页面联调与系统闭环 ✅
- Day 1
  - [x] 完成 `defense` 页面联调（列表、详情、审批、失败重试入口）。
- Day 2
  - [x] 完成 `scan` 页面联调（资产、任务、结果、筛选）。
- Day 3
  - [x] 完成 `ai-center` 页面联调（会话、报告、TTS）。
- Day 4
  - [x] 完成 `integrations/audit/settings` 页面联调。
- Day 5
  - [x] 跑通专项 E2E：权限矩阵、视图切换、通知已读、全局搜索、退出登录。
  - [x] 输出问题清单与下轮修复计划。
