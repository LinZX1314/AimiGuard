...
#### 成品化体验与内容要求（新增 TODO）
- [ ] U-1 视觉系统升级：重构 `frontend/src/style.css` 主题 token（主色、语义色、层级阴影、圆角），形成统一“安全运营控制台”视觉语言，减少当前通用模板感。
- [ ] U-2 仪表盘信息层级重构：将“核心风险指标 / 待处置任务 / 趋势洞察 / 操作入口”分区，避免纯卡片平铺导致重点不突出。
- [ ] U-3 空态/加载态/异常态规范：每个模块补齐 Skeleton、空态引导和错误恢复操作（重试/查看日志），禁止仅显示“暂无数据”。
...
- [ ] C-1 内容表达规范：所有关键卡片必须包含“当前值 + 环比/趋势 + 风险解释 + 下一步建议”，禁止只显示数字。
