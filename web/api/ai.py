...
def ai_chat_stream():
    ...
    def generate():
        ...
        if is_drill_mode and drill_state:
            ...
            yield f"data: {json.dumps({'done': True, 'session_id': sid, 'drill_report_link': f'/reports?sid={sid}'}, ensure_ascii=False)}\n\n"
            return
        ...
        if is_incident_mode and incident_state:
            ...
            yield f"data: {json.dumps({'done': True, 'session_id': sid, 'incident_report_link': f'/reports?sid={sid}'}, ensure_ascii=False)}\n\n"
            return
            ...
            if screenshot_path:
                try:
                    if os.path.exists(screenshot_path):
                        ...
                        img_tag = f"![截图](data:{mime_type};base64,{b64})"
                except Exception:
                    pass
            elif screenshot_url:
                img_tag = f"![截图]({screenshot_url})"
            report += f"| `{sr.get('ip', 'N/A')}` | {sr.get('port', 'N/A')} | {sr.get('url', 'N/A')} | {img_tag} |\n"
